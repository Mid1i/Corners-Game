...
void Engine::blackSet()
{
	...
	for (int i = 9; i < arraySize; i++)
	{
		...
		std::cout << mainArray[i]->pos.x << " " << mainArray[i]->pos.y << std::endl;
...
int Engine::checking(int x_pos, int y_pos)
{
	for (int i = 0; i < arraySize; i++)
	{
...
/*
	...
	ход возможен.
*/
bool Engine::canStay(int number, int index, int x_pos, int y_pos)
{
...
/*
	...
	удобства.
*/
void Engine::mouseStep(sf::RenderWindow &window, sf::Event &event)
{
	int j = 0;
